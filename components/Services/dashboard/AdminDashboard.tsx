...
import { Role, RepairRequest, RepairRequestStatus, User } from '../../../types';
...
export const AdminDashboard = ({ user }: { user: User }) => {
  ...
  const loadAdminStats = async () => {
    ...
    try {
      ...
      const allRequests: RepairRequest[] = await db.getAllRepairRequests();

      const totalUsers = allUsers.filter(u => u.role === Role.USER).length;
      const totalTechnicians = allUsers.filter(u => u.role === Role.TECHNICIAN).length;
      ...
      // Show up to 5 of the newest users
      setRecentUsers(
        [...allUsers].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)).slice(0, 5)
      );

      // Show up to 5 of the newest requests
      setRecentRequests(
        [...allRequests].sort((a, b) => {
          const rawA = (a as any).createdAt || 0;
          const rawB = (b as any).createdAt || 0;
          return rawB - rawA;
        }).slice(0, 5)
      );
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const loadVersion = async () => {
      await db.init();
      const saved = await db.getSetting('settings.apiVersion') || localStorage.getItem('settings.apiVersion') || 'v1';
      setApiVersion(saved);
    };
    loadVersion();
  }, []);
  ...
  const handleApiVersionChange = async (ver: string) => {
    ...
    await db.init();
    await db.setSetting('settings.apiVersion', ver);
  };
