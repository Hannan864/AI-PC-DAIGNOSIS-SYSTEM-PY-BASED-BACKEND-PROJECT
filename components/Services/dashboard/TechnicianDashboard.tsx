...
import { Technician, RepairRequest, RepairRequestStatus } from '../../../types';
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        const incomingCount = allRequests.filter(r => !r.technicianId && r.status === RepairRequestStatus.SUBMITTED).length;
        const techRequests = allRequests.filter(r => r.technicianId === user.id);
            ...
            {activeJobs.map(job => (
              <div key={job.id} className="p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                <div>
                  ...
                  <p className="text-xs text-slate-500">ID: {job.id.substr(0, 8)} • User: {job.userName}</p>
                </div>
