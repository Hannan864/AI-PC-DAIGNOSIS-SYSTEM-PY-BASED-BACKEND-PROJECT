...
export const AdminTechsMgmt: React.FC = () => {
  ...
  const handleToggleStatus = async (tech: User) => {
    ...
    try {
      await db.addUser(updatedTech);
      
      // Add entry to user history of tech to notify them or maintain logs
      await db.addUserHistory({
        historyId: `hist_${Date.now()}_${Math.random().toString(36).substr(2,6)}`,
        userId: tech.id,
        type: 'DIAGNOSTIC' as any,
        referenceId: 'DB_ADMIN_TOGGLE',
        title: `Technician Access Modified: ${newStatus}`,
        summary: `Platform Admin changed status clearance filter of technician profile to ${newStatus}`,
        timestamp: Date.now()
      });
