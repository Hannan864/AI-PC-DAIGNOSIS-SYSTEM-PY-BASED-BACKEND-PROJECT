...
export const AdminUsersMgmt: React.FC = () => {
  ...
  const handleToggleStatus = async (userToUpdate: User) => {
    ...
    try {
      ...
      // Log audit trace to user workspace history
      await db.addUserHistory({
        historyId: `hist_${Date.now()}_${Math.random().toString(36).substr(2,6)}`,
        userId: userToUpdate.id,
        type: 'DIAGNOSTIC' as any, // fallback type
        referenceId: 'SYSTEM_ADMIN_FLAG',
        title: `Account Soft Flag: ${newStatus}`,
        summary: `Admin set status of ${userToUpdate.name} to ${newStatus}`,
        timestamp: Date.now()
      });
      await loadData();
    } catch (err) {
