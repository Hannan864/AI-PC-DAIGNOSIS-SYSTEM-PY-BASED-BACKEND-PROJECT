...
export const RegisterScreen = ({ onRegisterSuccess, onGoToLogin }: { onRegisterSuccess: (sessionId: string) => void, onGoToLogin: () => void }) => {
  ...
  const { telemetryMode, setMode } = useTelemetryMode();
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await db.addUser({
        ...
        role,
        createdAt: Date.now(),
        ...(role === Role.TECHNICIAN ? { specialty: 'General Hardware' } : {})
      });
      ...
      await db.setSetting('current_session', sessionId);
    ...
    <div className="flex items-center justify-center h-full">
      <div className="w-full max-w-md p-8 glass rounded-2xl shadow-xl shadow-indigo-500/10">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div className="pt-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <button
                ...
                className={`py-2.5 rounded-lg border text-xs font-medium transition-all ${role === Role.ADMIN ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'bg-white/5 border-white/10 text-slate-400 hover:bg-white/10'}`}
              >
                Admin
              </button>
            </div>
          ...
          <div className="pt-2">
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3">Telemetry Source</label>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setMode('MOCK')}
                className={`py-2.5 px-3 rounded-lg border text-xs font-medium transition-all flex items-center justify-center gap-2 ${telemetryMode === 'MOCK' ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'bg-white/5 border-white/10 text-slate-400 hover:bg-white/10'}`}
              >
                <span className={`w-1.5 h-1.5 rounded-full ${telemetryMode === 'MOCK' ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'}`}></span>
                Mock Diagnostics
              </button>
              <button
                type="button"
                onClick={() => setMode('PYTHON')}
                className={`py-2.5 px-3 rounded-lg border text-xs font-medium transition-all flex items-center justify-center gap-2 ${telemetryMode === 'PYTHON' ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'bg-white/5 border-white/10 text-slate-400 hover:bg-white/10'}`}
              >
                <span className={`w-1.5 h-1.5 rounded-full ${telemetryMode === 'PYTHON' ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'}`}></span>
                Python Diagnostics
              </button>
            </div>
          </div>
