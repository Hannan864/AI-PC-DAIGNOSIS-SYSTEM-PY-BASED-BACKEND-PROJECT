...
export const ServiceSummaryView: React.FC<ServiceSummaryViewProps> = ({ req, onBack }) => {
  ...
  // PKR Costs
  const laborPKR = (report?.laborCost || 0) * EXCHANGE_RATE_PKR;
  const totalPKR = (report?.totalCost || 0) * EXCHANGE_RATE_PKR;
      ...
      <div className="glass rounded-2xl border border-white/10 overflow-hidden shadow-2xl bg-[#0b0f19]">
        ...
        <div className="p-6 bg-slate-900/60 border-b border-white/5 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
          ...
          <div className="text-left md:text-right font-mono shrink-0">
            ...
            <span className="text-2xl font-black text-white">${report?.totalCost || 0}</span>
            <span className="text-xs text-emerald-400 block font-semibold">
              ≈ {(report?.totalCost ? report.totalCost * EXCHANGE_RATE_PKR : 0).toLocaleString()} PKR
            </span>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="border border-white/5 rounded-xl bg-[#020617]/50 overflow-hidden">
                    ...
                    <table className="hidden sm:table w-full text-xs text-left">
                      ...
                      <tbody className="divide-y divide-white/5 text-slate-300 font-semibold">
                        ...
                        <tr>
                          ...
                          <td className="py-2 p-3 text-right text-slate-200 font-mono">${report.laborCost}</td>
                        </tr>
                        {/* Parts Row */}
                    ...
                    <div className="sm:hidden divide-y divide-white/5">
                      <div className="p-3 flex justify-between items-center text-xs">
                        ...
                        <span className="font-bold text-slate-200">${report.laborCost}</span>
                      </div>
                      {report.partsReplaced && report.partsReplaced.length > 0 ? (
                        report.partsReplaced.map((part, idx) => (
                          <div key={idx} className="p-3 space-y-1">
                            <div className="flex justify-between items-center text-xs">
                              ...
                              <span className="font-bold text-slate-200">${part.price * part.quantity}</span>
                            </div>
                      ...
                      <div className="p-4 bg-slate-950/40 flex justify-between items-center">
                        ...
                        <span className="text-lg font-black text-emerald-400 mono">${report.totalCost}</span>
                      </div>
                    </div>
                  </div>

                  {/* Currencies exchange conversion block */}
                  <div className="p-3 bg-slate-900/30 rounded-xl border border-white/5 flex justify-between items-center text-[10px] uppercase font-bold text-slate-500">
                    <span>PKR Invoice Conversion Rate:</span>
                    <span className="text-white font-mono">{ExchangeRateBreakdown(report.totalCost)}</span>
                  </div>
                </div>
              </div>
            </div>
...
// Helper for convert currency line formatting
function ExchangeRateBreakdown(usd: number): string {
  const cleanUsd = typeof usd === 'number' && !isNaN(usd) ? usd : 0;
  const converted = cleanUsd * EXCHANGE_RATE_PKR;
  return `$${cleanUsd} USD ≈ ${converted.toLocaleString()} PKR (Rate: 1 USD = ${EXCHANGE_RATE_PKR} PKR)`;
}
