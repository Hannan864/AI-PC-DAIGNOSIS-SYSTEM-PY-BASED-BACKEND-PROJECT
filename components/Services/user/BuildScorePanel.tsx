...
export const BuildScorePanel: React.FC<BuildScorePanelProps> = ({
  ...
  const handleExportReport = () => {
    const reportMarkdown = `# APCIE Build Report: ${buildName || 'Custom Blueprint'}
Generated: ${new Date().toLocaleDateString()}
Compatibility Standing: ${evalResult.status}
Performance Score: ${evalResult.score}/100

## Core Components Listing
- Processor (CPU): ${selectedCpu || 'Not Chosen'}
- Graphics Card (GPU): ${selectedGpu || 'Not Chosen'}
- Motherboard: ${selectedMotherboard || 'Not Chosen'}
- RAM Configuration: ${selectedRam || 'Not Chosen'}
- Main Storage Device: ${selectedStorage || 'Not Chosen'}
- Power Supply (PSU): ${selectedPowerSupply || 'Not Chosen'}
- Computer Chassis/Case: ${selectedCase || 'Not Chosen'}

## Financial Summary
- Combined Cost Bracket (USD): $${evalResult.estimatedUSDMin} - $${evalResult.estimatedUSDMax}
- Average Cost (PKR Rate): Rs. ${evalResult.estimatedPKR.toLocaleString()} (Calculated at 280 PKR/USD)

## Workload Suitability Vectors
- Gaming Efficiency & FPS: ${evalResult.gamingSuitability}%
- Creative Studio & 4K Editing: ${evalResult.editingSuitability}%
- Productivity & Office Suite: ${evalResult.officeSuitability}%

## Intelligent Optimization Diagnostics
${evalResult.issues.length > 0 ? '### Sizing & Technical Alerts:\n' + evalResult.issues.map(iss => `- [CRITICAL] ${iss}`).join('\n') : '- No static compatibility issues found.'}

${evalResult.bottlenecks.length > 0 ? '### Multi-Core Bottleneck Diagnostics:\n' + evalResult.bottlenecks.map(btn => `- [BOTTLENECK] ${btn}`).join('\n') : '- No bottlenecks detected between CPU, RAM or GPU speeds.'}

## System Power Safety Reserves
- Maximum Configuration Peak Load: ${evalResult.totalPowerWatts}W
- Selected Power Supply Rate: ${evalResult.psuMaxWatts}W
- Core Power Margin: ${evalResult.powerConsumptionSafetyMargin}% Headroom
`;
  ...
  return (
    <div id="build-score-panel" className="glass p-5 rounded-2xl border border-white/5 space-y-6 lg:sticky lg:top-6">
      {/* Price & Conversions */}
      <div>
        <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest pl-1 mb-2 font-mono">Pricing Framework (USD / PKR)</h4>
        <div className="px-3 py-3 bg-[#020617]/50 rounded-lg border border-white/5 space-y-1.5">
          <div className="flex items-end justify-between">
            <div>
              <span className="text-[9px] text-slate-500 block pb-0.5 font-mono">Budget Bracket (USD)</span>
              <span className="text-xl font-bold text-emerald-400">${evalResult.estimatedUSDMin} - ${evalResult.estimatedUSDMax}</span>
            </div>
            <div className="text-right">
              <span className="text-[9px] text-slate-500 pb-0.5 block font-mono">Est. Avg PKR</span>
              <span className="text-xs font-medium text-slate-300">Rs. {evalResult.estimatedPKR.toLocaleString()}</span>
            </div>
          </div>
          <p className="text-[9px] text-slate-500 border-t border-white/5 pt-1.5 font-mono">
            Exchange Rate Index lock: Rs. 280 / $1
          </p>
        </div>
      </div>
      ...
      <div className="pt-2 flex flex-col gap-2.5">
        <div className="flex gap-2">
          <button 
            onClick={onSave} 
            disabled={evalResult.status === 'FAIL'} 
          ...
          <button 
            onClick={onCancel} 
            className="px-4 py-2.5 bg-white/5 hover:bg-white/10 text-slate-300 rounded-lg text-sm font-medium transition-colors"
          >
            Cancel
          </button>
        </div>
