...
export const ServiceCompletionModal: React.FC<ServiceCompletionModalProps> = ({
  ...
  const [laborCost, setLaborCost] = useState<number>(45); // standard initial labor charge
  ...
  const finalTotalUSD = partsTotal + laborCost;
  const finalTotalPKR = finalTotalUSD * EXCHANGE_RATE_PKR;
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-0 md:p-4">
      <div className="bg-[#0b0f19] border-0 md:border md:border-white/10 md:rounded-2xl w-full h-full md:h-auto md:max-w-3xl shadow-2xl relative overflow-hidden flex flex-col max-h-screen md:max-h-[90vh]">
        ...
        <div className="p-5 border-b border-white/5 bg-slate-900/40 flex justify-between items-center shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-1 rounded-lg hover:bg-white/10 text-slate-500 hover:text-slate-300 transition-colors p-2"
          >
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6 flex-1 scrollbar-thin">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full bg-slate-950/70 border border-white/5 rounded-xl px-3.5 py-2 text-xs font-medium text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500/50"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-950/70 border border-white/5 rounded-xl px-3.5 py-2.5 text-xs font-medium text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500/50 resize-none"
                />
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Labor Charges ($ USD)</label>
                  <div className="relative w-28">
                    <span className="absolute inset-y-0 left-0 flex items-center pl-2.5 text-slate-500 text-xs font-semibold">$</span>
                    <input
                      type="number"
                      min={0}
                      required
                      ...
                      className="w-full pl-6 pr-2 py-1 bg-slate-950/70 border border-white/5 rounded-lg text-xs font-mono font-bold text-slate-200 focus:outline-none focus:border-indigo-500/50 text-right"
                    />
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-12 gap-1.5 items-end">
                  <div className="col-span-6 space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-white/5 rounded-md px-2 py-1 text-[10px] font-medium text-slate-300 placeholder-slate-700 focus:outline-none"
                    />
                  </div>
                  <div className="col-span-3 space-y-1">
                    <span className="text-[9px] uppercase font-bold tracking-wider text-slate-600 block">Unit price</span>
                    <input
                      type="number"
                      min={0}
                      placeholder="Price"
                      ...
                      className="w-full bg-slate-950 border border-white/5 rounded-md px-2 py-1 text-[10px] font-mono text-slate-300 focus:outline-none text-right"
                    />
                  </div>
                  <div className="col-span-2 space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-white/5 rounded-md px-2 py-1 text-[10px] text-slate-300 focus:outline-none text-center"
                    />
                  </div>
                  <div className="col-span-1 flex justify-end">
                    <button
                      ...
                      className="p-1 rounded bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[11px] leading-none text-center inline-flex items-center justify-center w-6 h-6 border border-white/5 shadow-md"
                      title="Add Part"
                    >
                      +
                    </button>
                    ...
                    {parts.map((p, idx) => (
                      <div key={idx} className="py-1.5 flex justify-between items-center text-[10.5px]">
                        <div className="truncate pr-2">
                          ...
                          <span className="text-slate-500 ml-1 font-mono">({p.quantity}x @ ${p.price})</span>
                        </div>
                        <div className="flex items-center gap-2 font-mono shrink-0">
                          <span className="text-slate-200">${p.price * p.quantity}</span>
                          <button
                            type="button"
                            onClick={() => handleRemovePart(idx)}
                            className="text-rose-400 hover:text-rose-200 text-[9px] font-bold"
                          >
                            ✕
                          </button>
                        </div>
              ...
              <div className="p-4 bg-emerald-500/5 rounded-xl border border-emerald-500/10 flex justify-between items-center">
                ...
                <div className="text-right">
                  <span className="text-lg font-black text-white block leading-none font-mono">${finalTotalUSD}</span>
          ...
          <div className="pt-4 border-t border-white/5 flex justify-end gap-3 shrink-0">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 bg-white/5 hover:bg-white/10 text-slate-300 font-semibold text-xs rounded-xl border border-white/5 transition-all"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-lg transition-all tracking-wider uppercase"
            >
