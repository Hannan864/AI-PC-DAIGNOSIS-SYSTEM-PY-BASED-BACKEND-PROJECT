...
export const ServiceReportsTech: React.FC = () => {
    ...
    const calculateReports = async () => {
      if (session?.userId) {
        ...
        try {
          ...
          completed.forEach(c => {
            const duration = Number(c.updatedAt) - Number(c.createdAt);
          ...
          const cleanAvgHours = isNaN(avgHours) ? 0 : avgHours;

          // Categorizations
          const hw = reqs.filter(r => r.issueCategory === 'HARDWARE_ISSUE').length;
          const sw = reqs.filter(r => r.issueCategory === 'SOFTWARE_ISSUE').length;
          const net = reqs.filter(r => r.issueCategory === 'NETWORK_ISSUE').length;

          setMetrics({
            totalAssigned: reqs.length,
            completedCount: completed.length,
            activeCount: active.length,
            avgTimeHours: parseFloat(cleanAvgHours.toFixed(1)),
            hardwareCount: hw,
            softwareCount: sw,
            networkCount: net
          });
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-500">Generating analytics telemetry report...</div>;
  }
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass p-5 rounded-2xl border border-white/5 space-y-2">
           ...
           <p className="text-xs text-slate-500">Successful, certified handoffs registered in IndexedDB files.</p>
        </div>
           ...
           <div className="space-y-4 pt-2">
              <div className="space-y-1">
                 ...
                 <div className="h-1.5 w-full bg-slate-900 rounded-full overflow-hidden">
                    <div className="h-full bg-rose-500 rounded-full" style={{ width: `${metrics.totalAssigned > 0 ? (metrics.hardwareCount / metrics.totalAssigned) * 100 : 0}%` }}></div>
                 </div>
              </div>
              ...
              <div className="space-y-1">
                 ...
                 <div className="h-1.5 w-full bg-slate-900 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-500 rounded-full" style={{ width: `${metrics.totalAssigned > 0 ? (metrics.softwareCount / metrics.totalAssigned) * 100 : 0}%` }}></div>
                 </div>
              </div>
              ...
              <div className="space-y-1">
                 ...
                 <div className="h-1.5 w-full bg-slate-900 rounded-full overflow-hidden">
                    <div className="h-full bg-amber-500 rounded-full" style={{ width: `${metrics.totalAssigned > 0 ? (metrics.networkCount / metrics.totalAssigned) * 100 : 0}%` }}></div>
                 </div>
              </div>
           </div>
        </div>

        {/* Detailed action logs list */}
                ...
                requests.slice(0, 5).map(req => (
                  <div key={req.id} className="flex justify-between items-center bg-[#020617]/50 rounded-xl p-3 border border-white/5 text-xs">
                     <div className="space-y-1 min-w-0 flex-1">
                        ...
                        <span className="text-slate-500 block">SLA update by Client: {req.userName}</span>
                     </div>

                     <span className="text-slate-400 font-sans font-medium shrink-0 ml-4">
                        State: <span className="font-bold text-indigo-400">{req.status}</span>
                     </span>
                  </div>
                ))
              )}
