...
// Lazy Components
const AdminDashboard = lazy(() => import('../Services/dashboard/AdminDashboard').then(m => ({ default: m.AdminDashboard })));
const ProfileScreen = lazy(() => import('../Services/auth/ProfileScreen').then(m => ({ default: m.ProfileScreen })));
const AdminUsersMgmt = lazy(() => import('../Services/admin/AdminUsersMgmt').then(m => ({ default: m.AdminUsersMgmt })));
const AdminTechsMgmt = lazy(() => import('../Services/admin/AdminTechsMgmt').then(m => ({ default: m.AdminTechsMgmt })));
const AdminRequestsMgmt = lazy(() => import('../Services/admin/AdminRequestsMgmt').then(m => ({ default: m.AdminRequestsMgmt })));
const AdminSystemReports = lazy(() => import('../Services/admin/AdminSystemReports').then(m => ({ default: m.AdminSystemReports })));
...
const AdminAppLayout: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      const savedTabs = await db.getSetting('admin_open_tabs');
      const lastActive = await db.getSetting('admin_active_tab');
  ...
  useEffect(() => {
    if (isReady) {
      db.setSetting('admin_open_tabs', tabs);
      db.setSetting('admin_active_tab', activeTabId);
    }
      ...
      <div className="p-4 flex items-center gap-3 border-b border-white/5 bg-rose-500/5">
        ...
        {(!isSidebarCollapsed || isMobileMenuOpen) && (
          <div className="flex flex-col min-w-0">
             ...
             <span className="text-[10px] text-rose-400 font-medium tracking-widest uppercase truncate">{user?.name}</span>
          </div>
        )}
      </div>
