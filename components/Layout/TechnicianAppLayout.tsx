...
// Lazy Components
const TechnicianDashboard = lazy(() => import('../Services/dashboard/TechnicianDashboard').then(m => ({ default: m.TechnicianDashboard })));
const ProfileScreen = lazy(() => import('../Services/auth/ProfileScreen').then(m => ({ default: m.ProfileScreen })));
const GigManagement = lazy(() => import('../Services/technician/GigManagement').then(m => ({ default: m.GigManagement })));
const IncomingRequestsTech = lazy(() => import('../Services/technician/IncomingRequestsTech').then(m => ({ default: m.IncomingRequestsTech })));
const IncomingBuildRequestsTech = lazy(() => import('../Services/technician/IncomingBuildRequestsTech').then(m => ({ default: m.IncomingBuildRequestsTech })));
const AssignedJobsTech = lazy(() => import('../Services/technician/AssignedJobsTech').then(m => ({ default: m.AssignedJobsTech })));
const ActiveRepairsTech = lazy(() => import('../Services/technician/ActiveRepairsTech').then(m => ({ default: m.ActiveRepairsTech })));
const CompletedJobsTech = lazy(() => import('../Services/technician/CompletedJobsTech').then(m => ({ default: m.CompletedJobsTech })));
const ServiceReportsTech = lazy(() => import('../Services/technician/ServiceReportsTech').then(m => ({ default: m.ServiceReportsTech })));
const CustomerHistoryTech = lazy(() => import('../Services/technician/CustomerHistoryTech').then(m => ({ default: m.CustomerHistoryTech })));
...
const TECH_SIDEBAR_GROUPS = [
  ...
  },
  {
    title: 'Repair Center',
    services: [
      ServiceType.INCOMING_REQUESTS, ServiceType.INCOMING_BUILD_REQUESTS, ServiceType.ASSIGNED_JOBS,
      ServiceType.ACTIVE_REPAIRS, ServiceType.COMPLETED_JOBS,
      ServiceType.GIG_MANAGEMENT,
      ServiceType.SERVICE_REPORTS, ServiceType.CUSTOMER_HISTORY
    ]
  },
  {
    title: 'Account',
...
const TechnicianAppLayout: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      const savedTabs = await db.getSetting('tech_open_tabs');
      const lastActive = await db.getSetting('tech_active_tab');
  ...
  useEffect(() => {
    if (isReady) {
      db.setSetting('tech_open_tabs', tabs);
      db.setSetting('tech_active_tab', activeTabId);
    }
      ...
      <div className="p-4 flex items-center gap-3 border-b border-white/5 bg-amber-500/5">
        ...
        {(!isSidebarCollapsed || isMobileMenuOpen) && (
          <div className="flex flex-col min-w-0">
             ...
             <span className="text-[10px] text-amber-400 font-medium tracking-widest uppercase truncate">{user?.name}</span>
          </div>
        )}
      </div>
